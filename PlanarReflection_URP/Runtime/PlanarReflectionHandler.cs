...
namespace PMP {
    public class PlanarReflectionHandler : MonoBehaviour {
        ...
        // 反射用パネルオブジェクト
        public GameObject reflectionPanel = null;
        public Transform reflectionPanelUpOverride = null;

        public Transform GetPanelTransform() {
            if (reflectionPanelUpOverride) return reflectionPanelUpOverride;
            if (reflectionPanel) return reflectionPanel.transform;
            return transform;
        }
        ...
        public int refCamRenderInterval = 0;
        int tmpFrameCount = 0;
        ...
        void Initialize() {
            ...
            {   // メインカメラ
                ...
                if (mainCamera) {
                    mainCameraTrns = mainCamera.transform;
                } else {
                    Debug.LogError("カメラの取得に失敗しました。");
                    enabled = false;
                    return;
                }
            }
            ...
            {   // フレームブレンド
                if(!frameBlendShader) {
                    Debug.LogError("フレームブレンド用のシェーダーが見つかりませんでした。");
                    enabled = false;
                    return;
                }
            ...
            {   // ブラー
                if (!kernelBlurShader) {
                    Debug.LogError("ブラー用のシェーダーが見つかりませんでした。");
                    enabled = false;
                    return;
                }
